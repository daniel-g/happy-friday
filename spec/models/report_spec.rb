...
describe Report do
  ...
  context 'many teams in different timezones: hard case' do
    ...
    it 'generates a report of tasks sorted by hour of work' do
      expect(data.size).to eq(Task.count)
      expect(data[0]).to eq(to_data_hash([moscow.name, '9:00am - 1:00pm', '6:00am - 10:00am', moscow_1.external_id]))
      expect(data[1]).to eq(to_data_hash([london.name, '9:00am - 2:00pm', '9:00am - 2:00pm', london_1.external_id]))
      expect(data[2]).to eq(to_data_hash([moscow.name, '1:00pm - 7:00pm', '10:00am - 4:00pm', moscow_2.external_id]))
      expect(data[3]).to eq(to_data_hash([london.name, '2:00pm - 6:00pm', '2:00pm - 6:00pm', london_2.external_id]))
    end
  end
